...
final class CustomActionTests: XCTestCase {
    ...
    func testDecodeGNOMEShapedJSONWithDefaults() throws {
        ...
        XCTAssertEqual(actions[0].outputBehavior, .preview)
        XCTAssertTrue(actions[0].visibilityConditions.isEmpty)
        XCTAssertNil(actions[0].sourceCatalogID)
    ...
    func testRoundTripThroughEncode() {
        let action = CustomAction(
            ...
            outputBehavior: .insertAfterSelection,
            visibilityConditions: [.editableTextOnly, .minimumLength(4)],
            sourceCatalogID: "catalog-translate")
        let decoded = CustomActionStore.decode(CustomActionStore.encode([action]))
        XCTAssertEqual(decoded, [action])
        XCTAssertEqual(decoded.first?.sourceCatalogID, "catalog-translate")
    }

    func testAllOutputBehaviorsDecode() {
        for behavior in ActionOutputBehavior.allCases {
            let action = CustomAction(
                name: behavior.title,
                prompt: "Do it",
                outputBehavior: behavior)
            let decoded = CustomActionStore.decode(CustomActionStore.encode([action]))
            XCTAssertEqual(decoded.first?.outputBehavior, behavior)
        }
    }
