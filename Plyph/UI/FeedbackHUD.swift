import AppKit

@MainActor
protocol ActionFeedbackSurface: AnyObject {
    var canPresentFeedback: Bool { get }
    func presentFeedback(
        _ message: String,
        isError: Bool,
        isWorking: Bool,
        duration: TimeInterval)
    func dismissFeedback()
}

/// Fallback feedback for actions without an originating surface. Its location
/// is captured when work begins and remains fixed for the whole operation.
@MainActor
final class FeedbackHUD: NSObject {
    static let shared = FeedbackHUD()

    private var panel: NSPanel?
    private var effectView: NSVisualEffectView?
    private var label: NSTextField?
    private var iconView: NSImageView?
    private var hideTask: Task<Void, Never>?
    private var fixedAnchor: NSPoint?
    private weak var anchoredSurface: ActionFeedbackSurface?

    private override init() {
        super.init()
    }

    var hasAnchoredSurface: Bool {
        anchoredSurface?.canPresentFeedback == true
    }

    func attach(to surface: ActionFeedbackSurface) {
        anchoredSurface = surface
        dismissPanel()
    }

    func show(_ message: String, isError: Bool, duration: TimeInterval) {
        let settings = SettingsStore.shared
        guard settings.pointerFeedback || isError else { return }
        let isWorking = duration == 0 && !isError
        if let anchoredSurface, anchoredSurface.canPresentFeedback {
            dismissPanel()
            anchoredSurface.presentFeedback(
                message,
                isError: isError,
                isWorking: isWorking,
                duration: duration)
            return
        }
        anchoredSurface = nil
        present(message: message, isError: isError, duration: duration)
    }

    func dismiss() {
        anchoredSurface?.dismissFeedback()
        anchoredSurface = nil
        dismissPanel()
    }

    private func dismissPanel() {
        hideTask?.cancel()
        hideTask = nil
        panel?.orderOut(nil)
        fixedAnchor = nil
    }

    private var stackView: NSStackView?

    private func present(message: String, isError: Bool, duration: TimeInterval) {
        hideTask?.cancel()
        hideTask = nil

        if panel == nil {
            createPanel()
        }
        guard
            let panel,
            let effectView,
            let label,
            let iconView
        else { return }

        let font = NSFont.systemFont(ofSize: 13, weight: .medium)
        let attributes: [NSAttributedString.Key: Any] = [
            .font: font,
            .foregroundColor: NSColor.labelColor,
        ]
        let attributed = NSAttributedString(string: message, attributes: attributes)
        let maxTextWidth: CGFloat = 300
        let fitting = attributed.boundingRect(
            with: NSSize(width: maxTextWidth, height: .greatestFiniteMagnitude),
            options: [.usesLineFragmentOrigin, .usesFontLeading])

        let horizontalPadding: CGFloat = 16
        let iconWidth: CGFloat = 16
        let gap: CGFloat = 8
        let contentWidth = min(maxTextWidth, ceil(fitting.width))
        let width = max(120, contentWidth + horizontalPadding * 2 + iconWidth + gap)
        let height = max(38, ceil(fitting.height) + 18)
        let size = NSSize(width: width, height: height)

        panel.setFrame(NSRect(origin: panel.frame.origin, size: size), display: false)
        effectView.frame = NSRect(origin: .zero, size: size)

        label.attributedStringValue = attributed

        let symbolName: String
        if isError {
            symbolName = "exclamationmark.circle.fill"
            iconView.contentTintColor = .systemRed
        } else if message.lowercased().contains("working") {
            symbolName = "sparkles"
            iconView.contentTintColor = .secondaryLabelColor
        } else {
            symbolName = "checkmark.circle.fill"
            iconView.contentTintColor = .systemGreen
        }
        iconView.image = NSImage(
            systemSymbolName: symbolName,
            accessibilityDescription: isError ? "Error" : "Status")

        if panel.isVisible == false || fixedAnchor == nil {
            fixedAnchor = NSEvent.mouseLocation
        }
        position()

        panel.alphaValue = 0
        panel.orderFrontRegardless()
        NSAnimationContext.runAnimationGroup { context in
            context.duration = 0.12
            panel.animator().alphaValue = 1
        }

        if duration > 0 {
            hideTask = Task { [weak self] in
                try? await Task.sleep(
                    nanoseconds: UInt64(duration * 1_000_000_000))
                guard let self, !Task.isCancelled, let panel = self.panel else { return }

                await NSAnimationContext.runAnimationGroup { context in
                    context.duration = 0.15
                    panel.animator().alphaValue = 0
                }
                try? await Task.sleep(nanoseconds: 170_000_000)
                guard !Task.isCancelled else { return }
                panel.orderOut(nil)
                self.fixedAnchor = nil
                self.hideTask = nil
            }
        }
    }

    private func createPanel() {
        let panel = NSPanel(
            contentRect: NSRect(x: 0, y: 0, width: 180, height: 38),
            styleMask: [.borderless, .nonactivatingPanel],
            backing: .buffered,
            defer: false)
        panel.isOpaque = false
        panel.backgroundColor = .clear
        panel.level = .floating
        panel.hasShadow = true
        panel.collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary]
        panel.isReleasedWhenClosed = false
        panel.ignoresMouseEvents = true

        let effect = NSVisualEffectView(frame: panel.contentView?.bounds ?? .zero)
        effect.autoresizingMask = [.width, .height]
        effect.material = .popover
        effect.state = .active
        effect.blendingMode = .behindWindow
        effect.wantsLayer = true
        effect.layer?.cornerRadius = 12
        effect.layer?.cornerCurve = .continuous
        effect.layer?.masksToBounds = true

        // Let the system popover material and native window shadow provide
        // separation. An explicit separator border looks heavy on light content
        // and does not adapt as naturally across macOS appearances.

        let icon = NSImageView(frame: .zero)
        icon.imageScaling = .scaleProportionallyDown
        icon.translatesAutoresizingMaskIntoConstraints = false
        icon.widthAnchor.constraint(equalToConstant: 16).isActive = true
        icon.heightAnchor.constraint(equalToConstant: 16).isActive = true

        let label = NSTextField(labelWithString: "")
        label.isEditable = false
        label.isSelectable = false
        label.drawsBackground = false
        label.isBordered = false
        label.alignment = .center
        label.lineBreakMode = .byWordWrapping
        label.maximumNumberOfLines = 2
        label.translatesAutoresizingMaskIntoConstraints = false

        let stack = NSStackView(views: [icon, label])
        stack.orientation = .horizontal
        stack.alignment = .centerY
        stack.distribution = .fill
        stack.spacing = 8
        stack.translatesAutoresizingMaskIntoConstraints = false

        effect.addSubview(stack)
        panel.contentView = effect

        NSLayoutConstraint.activate([
            stack.centerXAnchor.constraint(equalTo: effect.centerXAnchor),
            stack.centerYAnchor.constraint(equalTo: effect.centerYAnchor),
            stack.leadingAnchor.constraint(greaterThanOrEqualTo: effect.leadingAnchor, constant: 14),
            stack.trailingAnchor.constraint(lessThanOrEqualTo: effect.trailingAnchor, constant: -14)
        ])

        self.panel = panel
        self.effectView = effect
        self.iconView = icon
        self.label = label
        self.stackView = stack
    }

    private func position() {
        guard let panel else { return }
        let anchor = fixedAnchor ?? NSEvent.mouseLocation
        var x = anchor.x + 18
        var y = anchor.y - panel.frame.height - 18

        for screen in NSScreen.screens where screen.frame.contains(anchor) {
            let visible = screen.visibleFrame
            x = min(
                max(x, visible.minX + 8),
                visible.maxX - panel.frame.width - 8)
            y = min(
                max(y, visible.minY + 8),
                visible.maxY - panel.frame.height - 8)
            break
        }

        panel.setFrameOrigin(NSPoint(x: x, y: y))
    }
}
