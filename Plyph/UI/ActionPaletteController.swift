...
final class ActionPaletteController: NSObject {
    ...
    func show(
        ...
        modeHandler: @escaping (RunMode) -> Void,
        askHandler: @escaping () -> Void
    ) {
    ...
    private func buildItems(context: SelectionContext?) {
        let settings = SettingsStore.shared
        let descriptors = ActionCatalog.all(settings: settings)
            .filter { $0.isVisible(in: context) }
        let builtIns = descriptors.filter { $0.reference.customID == nil }
        let custom = descriptors.filter { $0.reference.customID != nil }

        items = builtIns.compactMap { descriptor in
            PaletteItem(
                name: descriptor.name,
                icon: descriptor.icon,
                mode: descriptor.mode ?? .prompt,
                isSeparator: false,
                isAsk: descriptor.isAsk)
        }
        if !custom.isEmpty {
            items.insert(
                PaletteItem(name: "", icon: nil, mode: .correct, isSeparator: true),
                at: items.count)
            items += custom.compactMap { descriptor in
                guard let mode = descriptor.mode else { return nil }
                return PaletteItem(
                    name: descriptor.name,
                    icon: descriptor.icon,
                    mode: mode,
    ...
    private func buildPanel() {
        ...
        // Match the density of a native contextual menu: just enough room for
        // the icon, title and menu insets, rather than a fixed wide palette.
        let paletteWidth = min(300, max(200, ceil(widestTitle + 46)))
        ...
        let paletteHeight = min(listHeight + outerPadding * 2, maxHeight)
        ...
        column.width = paletteWidth - outerPadding * 2
        ...
        scroll.hasVerticalScroller = listHeight > maxHeight - outerPadding * 2
        ...
        NSLayoutConstraint.activate([
            scroll.topAnchor.constraint(equalTo: effect.topAnchor, constant: outerPadding),
            scroll.leadingAnchor.constraint(equalTo: effect.leadingAnchor, constant: outerPadding),
            scroll.trailingAnchor.constraint(equalTo: effect.trailingAnchor, constant: -outerPadding),
            scroll.bottomAnchor.constraint(equalTo: effect.bottomAnchor, constant: -outerPadding),
        ])
...
extension ActionPaletteController: NSTableViewDataSource, NSTableViewDelegate {
    ...
    func tableView(_ tableView: NSTableView, heightOfRow row: Int) -> CGFloat {
        items[row].isSeparator ? separatorHeight : rowHeight
    }
