...
final class SelectionDotController: NSObject {
    ...
    private var onAction: ((ActionReference, NSRunningApplication?) -> Void)?
    private var onMore: ((NSRunningApplication?, SelectionContext) -> Void)?
    ...
    func start(
        onAction: @escaping (ActionReference, NSRunningApplication?) -> Void,
        onMore: @escaping (NSRunningApplication?, SelectionContext) -> Void
    ) {
    ...
    private func installMonitors() {
        ...
        globalMouseUpMonitor = NSEvent.addGlobalMonitorForEvents(
            ...
            Task { @MainActor in
                if self.toolbar.isShowingActionStatus {
                    self.mouseDownLocation = nil
                    return
                }
                // The toolbar expands on mouse-down. Ignore the matching mouse-up
                // so it is not mistaken for a newly completed text selection.
                if self.toolbar.isVisible, self.toolbar.frame.contains(loc) {
                    self.mouseDownLocation = nil
                    return
                }
        ...
        localMouseUpMonitor = NSEvent.addLocalMonitorForEvents(
            ...
            Task { @MainActor in
                ...
                // Keep interactions with our panel out of selection detection.
                if self.toolbar.isVisible, self.toolbar.frame.contains(loc) {
                    self.mouseDownLocation = nil
                    return
                }
        ...
        globalDismissMonitor = NSEvent.addGlobalMonitorForEvents(matching: dismissMask) { [weak self] event in
            ...
            guard self.toolbar.isVisible else { return }
            if event.type == .keyDown && event.keyCode == 53 {
                Task { @MainActor in self.toolbar.handleEscape() }
                return
            }
            if self.toolbar.isShowingActionStatus { return }
            if event.type == .leftMouseDown || event.type == .rightMouseDown || event.type == .otherMouseDown {
                let mouseLoc = NSEvent.mouseLocation
                if self.toolbar.frame.contains(mouseLoc) {
                    return
                }
            }
            Task { @MainActor in self.hideDot() }
        ...
        localDismissMonitor = NSEvent.addLocalMonitorForEvents(matching: dismissMask) { [weak self] event in
            ...
            guard self.toolbar.isVisible else { return event }
            if event.type == .keyDown && event.keyCode == 53 {
                Task { @MainActor in self.toolbar.handleEscape() }
                return event
            }
            if self.toolbar.isShowingActionStatus { return event }
            if event.type == .leftMouseDown || event.type == .rightMouseDown || event.type == .otherMouseDown {
                let mouseLoc = NSEvent.mouseLocation
                if self.toolbar.frame.contains(mouseLoc) {
                    return event
        ...
        let obs1 = nc.addObserver(
            ...
            Task { @MainActor in
                guard let self, !self.toolbar.isShowingActionStatus else { return }
                self.hideDot()
            }
        }
    ...
    private func evaluateCurrentSelection(
        ...
        let context = SelectionContext(
            text: selectedText(in: selectedElement) ?? "",
            bundleIdentifier: frontApp.bundleIdentifier,
            isEditable: isEditable(element: selectedElement))
        showDot(
            near: anchor,
            on: screen(containing: mouseLocation),
            context: context,
            targetApp: frontApp)
    }
    ...
    private func selectedText(in element: AXUIElement) -> String? {
        if let text = AXElement.selectedText(of: element),
           !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return text
        }
        var markerRef: CFTypeRef?
        ...
        for attribute in [
            ...
            var value: CFTypeRef?
            if AXUIElementCopyParameterizedAttributeValue(
                element, attribute as CFString, markerRef, &value) == .success {
    ...
    private func isEditable(element: AXUIElement) -> Bool {
        if AXElement.selectedTextSettable(element) { return true }
        var value: CFTypeRef?
        if AXUIElementCopyAttributeValue(
            element, "AXEditable" as CFString, &value) == .success,
           let editable = value as? Bool {
            return editable
        }
        return false
    }
    ...
    private func showDot(
        ...
        let actions = SettingsStore.shared.resolvedQuickActions(for: context)
        toolbar.show(
            near: anchor,
            on: screen,
            actions: actions,
            onAction: { [weak self] reference in
                guard let self else { return }
                if SettingsStore.shared.pointerFeedback {
                    self.toolbar.beginAction(reference)
                    FeedbackHUD.shared.attach(to: self.toolbar)
                } else {
                    self.toolbar.hide()
                }
                self.onAction?(reference, targetApp)
            },
            onMore: { [weak self] in
                self?.onMore?(targetApp, context)
            })
    }
