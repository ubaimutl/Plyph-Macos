...
enum ModelCatalog {
    ...
    private static func fetchOllama(config: AiConfig) async throws -> [ModelOption] {
        let rawBase = config.ollamaURL
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !rawBase.isEmpty else {
            throw PromptError.network("Add your Ollama server URL first.")
        }

        var normalizedBase = rawBase
        ...
        guard let baseURL = URL(string: normalizedBase),
            ...
            throw PromptError.network(
                "Invalid Ollama URL. Use a URL such as http://localhost:11434.")
        }

        let tagsURL = baseURL.appendingPathComponent("api/tags")
        let data = try await getJSON(
            url: tagsURL, headers: [:], providerName: "Ollama")
