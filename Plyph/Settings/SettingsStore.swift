...
final class SettingsStore: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        quickActionConfiguration = ActionConfigurationStore.decode(
            QuickActionConfiguration.self,
            from: defaults.string(forKey: "quick-actions") ?? "")
            ?? .default
        appActionProfiles = ActionConfigurationStore.decode(
            [AppActionProfile].self,
            from: defaults.string(forKey: "app-action-profiles") ?? "")
            ?? []
    ...
    static func registerDefaults(in defaults: UserDefaults) {
        defaults.register(defaults: [
            ...
            "quick-actions": ActionConfigurationStore.encode(
                QuickActionConfiguration.default),
            "app-action-profiles": "[]",
            "model-cache": "{}",
    ...
    @Published var quickActionConfiguration: QuickActionConfiguration {
        didSet {
            let normalized = QuickActionConfiguration.normalized(
                quickActionConfiguration.actions)
            if normalized != quickActionConfiguration.actions {
                quickActionConfiguration = QuickActionConfiguration(actions: normalized)
                return
            }
            defaults.set(
                ActionConfigurationStore.encode(quickActionConfiguration),
                forKey: "quick-actions")
        }
    }

    @Published var appActionProfiles: [AppActionProfile] {
        didSet {
            let normalized = Self.normalizedProfiles(appActionProfiles)
            if normalized != appActionProfiles {
                appActionProfiles = normalized
                return
            }
            defaults.set(
                ActionConfigurationStore.encode(appActionProfiles),
                forKey: "app-action-profiles")
        }
    }
    ...
    var availableActionDescriptors: [ActionDescriptor] {
        ActionCatalog.all(settings: self)
    }

    func resolvedQuickActions(for context: SelectionContext) -> [ActionDescriptor] {
        ActionCatalog.quickActions(settings: self, context: context)
    }

    func setProfile(_ profile: AppActionProfile) {
        guard !profile.bundleIdentifier.isEmpty else { return }
        if let index = appActionProfiles.firstIndex(where: {
            $0.bundleIdentifier.caseInsensitiveCompare(profile.bundleIdentifier) == .orderedSame
        }) {
            appActionProfiles[index] = profile
        } else {
            appActionProfiles.append(profile)
        }
    }

    func removeProfile(bundleIdentifier: String) {
        appActionProfiles.removeAll {
            $0.bundleIdentifier.caseInsensitiveCompare(bundleIdentifier) == .orderedSame
        }
    }

    private static func normalizedProfiles(_ profiles: [AppActionProfile])
        -> [AppActionProfile]
    {
        var seen = Set<String>()
        return profiles.compactMap { profile in
            let normalized = AppActionProfile(
                bundleIdentifier: profile.bundleIdentifier,
                actions: profile.actions)
            guard !normalized.bundleIdentifier.isEmpty,
                  seen.insert(normalized.bundleIdentifier).inserted else { return nil }
            return normalized
        }
    }
