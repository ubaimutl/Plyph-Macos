...
enum PlyphDebug {
    private static let queue = DispatchQueue(label: "com.ubaimutl.Plyph.debug-log")

    static let logURL: URL = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Logs/Plyph", isDirectory: true)
    ...
    static func log(_ message: String, session: String? = nil) {
        ...
        NSLog("[PlyphDebug]%@ %@", sessionPart, message)
        ...
        queue.async {
            do {
                ...
                NSLog("[PlyphDebug] Failed to write debug log: %@", error.localizedDescription)
            }
        }
    }
...
enum KeyPoster {
    ...
    static func postCopy(
        ...
        PlyphDebug.log(
            "postCopy route=\(useAppleScript ? "SystemEvents" : "CGEvent") target={\(PlyphDebug.appSummary(app))} frontmost={\(PlyphDebug.frontmostSummary())}",
    ...
    static func postPaste(
        ...
        let useAppleScript = shouldUseAppleScript(for: app)
        PlyphDebug.log(
            "postPaste route=\(useAppleScript ? "SystemEvents" : "CGEvent") target={\(PlyphDebug.appSummary(app))} frontmost={\(PlyphDebug.frontmostSummary())}",
            session: debugSession)
        if useAppleScript {
    ...
    static func postPasteUsingSystemEvents(debugSession: String? = nil) -> Bool {
        PlyphDebug.log(
            "postPaste route=SystemEvents(forced) frontmost={\(PlyphDebug.frontmostSummary())}",
            session: debugSession)
        return postAppleScriptCommandKey(keyChar: "v", debugSession: debugSession)
    }
    ...
    private static func postAppleScriptCommandKey(
        ...
        PlyphDebug.log(
            "System Events AppleScript BEGIN key=\(keyChar) frontmost={\(PlyphDebug.frontmostSummary())}",
            session: debugSession)
        ...
        if let error {
            PlyphDebug.log(
                "System Events AppleScript FAILED error=\(String(describing: error)) frontmost={\(PlyphDebug.frontmostSummary())}",
                session: debugSession)
            return false
        }

        PlyphDebug.log(
            "System Events AppleScript SUCCESS descriptor=\(result.stringValue ?? "<no string result>") frontmost={\(PlyphDebug.frontmostSummary())}",
            session: debugSession)
        return true
    }
    ...
    private static func postCommandKey(
        ...
        PlyphDebug.log(
            "CGEvent POSTED keyCode=\(key) frontmost={\(PlyphDebug.frontmostSummary())}",
            session: debugSession)
        return true
    }
}
