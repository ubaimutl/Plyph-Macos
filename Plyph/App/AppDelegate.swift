...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @MainActor func applySelectionDotSetting() {
        if SettingsStore.shared.selectionDotEnabled {
            SelectionDotController.shared.start(
                onAction: { [weak self] reference, targetApp in
                    self?.runner?.run(action: reference, targetApp: targetApp)
                },
                onMore: { [weak self] targetApp, context in
                    self?.runner?.openActionPalette(
                        forcePopup: true,
                        targetApp: targetApp,
                        context: context)
