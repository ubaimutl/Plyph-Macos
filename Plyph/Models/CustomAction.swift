...
struct CustomAction: Codable, Identifiable, Equatable {
    ...
    /// Empty means Always. Multiple conditions are combined with AND.
    var visibilityConditions: [ActionVisibilityCondition]
    /// The immutable catalog template this editable action was created from.
    /// Nil for actions created manually and for legacy saved actions.
    ...
    init(
        ...
        inputLimit: Int = 0,
        outputLimit: Int = 0,
        outputBehavior: ActionOutputBehavior = .preview,
        visibilityConditions: [ActionVisibilityCondition] = [],
        ...
        self.visibilityConditions = visibilityConditions.filter { $0 != .always }
    ...
    init(from decoder: Decoder) throws {
        ...
        outputBehavior = (try? container.decodeIfPresent(
            ActionOutputBehavior.self,
            forKey: .outputBehavior)) ?? .preview
        visibilityConditions = (try? container.decodeIfPresent(
            [ActionVisibilityCondition].self,
            forKey: .visibilityConditions)) ?? []
    ...
    var summarySubtitle: String {
        ...
        if outputBehavior != .preview {
            parts.append(outputBehavior.title)
        }
        if !visibilityConditions.isEmpty {
            parts.append(visibilityConditions.map(\.title).joined(separator: ", "))
        }
